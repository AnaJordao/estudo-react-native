...
function ExpenseForm({ submitButtonLabel, defaultValues, onCancel, onSUbmit }) {
  ...
  function submitHandler() {
    ...
    const dateIsValue = expenseData.date.toString() !== "Invalid Date";
    const descriptionIsValid = expenseData.description.trim().length > 0; // trim so it removes any white spaces

    if (!amountIsValid || !dateIsValue || !descriptionIsValid) {
